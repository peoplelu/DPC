...
def init_parse_argparse_default_params(parser, dataset_name=None, arch=None):
    ...
    parser.add_argument("--task_name", type=str, default="shape_corr", choices=TASK_OPTIONS, help="The task to solve")
    ...
    parser.add_argument("--test_on_shrec", type=str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--test_on_tosca", type=str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--test_on_surreal", type=str2bool, nargs="?", const=True, default=False)
    parser.add_argument(
        "--rotate_factor",
        type=float,
        default=40,
    ...
    parser.add_argument(
        "--show_corr_vis", type=str2bool, nargs="?", const=True, default=True, help="If true logs visualizations of pair correspondence (run time)"
    )
    parser.add_argument(
        "--rotate_pc_for_vis", type=str2bool, nargs="?", const=True, default=True, help="If true, rotate point cloud for visualization"
    )
    parser.add_argument(
        "--rotate_pc_angles", nargs='+', default=[0, 0, 0], help="Rotation angle (in degrees) of point cloud for visualization"
    )
    parser.add_argument(
        "--log_html", type=str2bool, nargs="?", const=True, default=False, help="If true logs the html assests as well (storage)"
    )
    parser.add_argument(
        "--write_image", type=str2bool, nargs="?", const=True, default=True, help="If true, write the image assests as well (for offline logger) (storage)"
    )
    parser.add_argument(
        "--write_html", type=str2bool, nargs="?", const=True, default=False, help="If true, write the html assests as well (for offline logger) (storage)"
    )
    parser.add_argument("--vis_mitsuba", type=str2bool, nargs="?", const=True, default=False, help="If true, write mitsuba xml file for rendering")
    parser.add_argument("--mitsuba_debug_mode", type=str2bool, nargs="?", const=True, default=True)
    parser.add_argument("--save_data_for_vis", nargs="?", default=False, type=str2bool, const=True, help="Whether to save data for visualization (storage)")
    parser.add_argument('--vis_idx_list', nargs='+', default=[], help='List of batch indices for visualization (empty list [] for all batches)')
    parser.add_argument(
        "--do_train", type=str2bool, nargs="?", const=True, default=True, help="Train the model"
    )
    parser.add_argument("--predict", type=str2bool, nargs="?", const=True, default=False)
    parser.add_argument(
        "--display_id", type=int,help="For headless mutlithreaded we might want to specify display numbe"
    )
    parser.add_argument("--train_val_split", default=0.8, type=float, )
    parser.add_argument("--test_during_train", type=str2bool, nargs="?", const=True, default=True)
    ...
    parser.add_argument(
        "--dropout",  default={"complition": 0.3, "shape_corr": 0.3}[task_name], help="The amount of features to drop"
    )
    parser.add_argument(
        "--latent_dim",
    ...
    parser.add_argument("--offline_logger", type=str2bool, nargs="?", const=True, default=True)
    parser.add_argument("--OVERFIT_singel_pair", default=None, type=str, help="Should be set in the format : 8815_9583 And this will be the only pair in the training")
    ...
    parser.add_argument("--flush_logs_every_n_steps", default=1, type=int, help="flush_logs_every_n_steps")
    parser.add_argument("--log_every_n_steps", default=50, type=int, help="log_every_n_steps")
    parser.add_argument("--metric_to_track", default='val_tot_loss', type=str, help="The metric the checkpoint manager will track")
    parser.add_argument("--metric_score_cutoff", default=0.05, type=float, help="The result(min or max) to kill the run if not achieved in K minutes")

    ##misc
    parser.add_argument("--without_logger", action="store_true", help="If true, will not log")
